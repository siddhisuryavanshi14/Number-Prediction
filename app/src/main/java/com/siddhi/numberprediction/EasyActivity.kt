...
class EasyActivity : AppCompatActivity() {
        ...
        btnStart.setOnClickListener {
            ...
            mDialogView.btnCheck.setOnClickListener { v ->
                ...
                if (mDialogView.etInput.text.toString().trim().isNotEmpty()) {
                    ...
                    if (10 <= etInput.toInt() && etInput.toInt() < 100)
                        availCount -= 1
                    if (availCount > 0) {
                        mDialogView.txtCount.setText("Attempts = $availCount")
                            val imm =
                            ...
                            if (10 <= etInput.toInt() && etInput.toInt() < 100) {
                                if (number > etInput.toInt()) {
                                    ...
                                    var prevHighScore: String? =
                                        sharedPreferences.getString("newHighEasy", "0")
                                    alert.dismiss()
                                ...
                                Toast.makeText(
                                    this,
                                    "Please enter a number between 10 to 99",
                                    Toast.LENGTH_LONG
                                ).show()
                            }
    ...
    fun savePreferences(name: String,attempts:String) {
        sharedPreferences.edit().putString("nameEasy",name).apply()
        sharedPreferences.edit().putString("attemptsEasy",attempts).apply()
    }
