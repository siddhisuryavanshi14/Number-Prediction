...
class NormalActivity : AppCompatActivity() {
        ...
        btnStart.setOnClickListener {
            ...
            mDialogView.btnCheck.setOnClickListener { v ->
                ...
                if (mDialogView.etInput.text.toString().trim().isNotEmpty()) {
                    ...
                    if (100 <= etInput.toInt() && etInput.toInt() < 1000)
                        availCount -= 1
                    if (availCount > 0) {
                        mDialogView.txtCount.setText("Attempts = $availCount")
                        val imm =
                        ...
                        if (100 <= etInput.toInt() && etInput.toInt() < 1000) {
                            if (number > etInput.toInt()) {
                                ...
                                var prevHighScore: String? =
                                    sharedPreferences.getString("newHighNormal", "0")
                                alert.dismiss()
    ...
    fun savePreferences(name: String,attempts:String) {
        sharedPreferences.edit().putString("nameNormal",name).apply()
        sharedPreferences.edit().putString("attemptsNormal",attempts).apply()
    }
}
